...
var initAppPage = function (pageParams) {
    ...
    // 初始化重力感应场景
    if (baseParams.sceneGravityRoot) {
        initSceneGravity({
            rootElem: baseParams.sceneGravityRoot
        });
    }
};
...
// 初始化重力感应场景
var initSceneGravity = function (sceneGravityParams) {
    var baseParams = {
        rootElem: null
    };
    if ($.isPlainObject(sceneGravityParams)) {
        $.extend(baseParams, sceneGravityParams);
    }
    if (baseParams.rootElem) {
        var opt = {
            calibrateX: false,
            calibrateY: true,
            invertX: false,
        ...
        var gravityScenes = $(baseParams.rootElem);
        $.each(gravityScenes, function (sceneIndex, scene) {
            $(scene).parallax();
        });
    }
};
